...
async function cargarDatos() {
  try {
    ...
    if (presupuestosLS) {
      ...
      const resPresupuestos = await fetch('./data/presupuestos.json');
      presupuestos = await resPresupuestos.json();
...
form.addEventListener("submit", (e) => {
  ...
  form.reset();
  selectSalon.value = "";
  divServicios.querySelectorAll("input[type='checkbox']").forEach(c => c.checked = false);
});
